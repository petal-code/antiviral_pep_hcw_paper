# westAfrica_run_abc_calibration.R  (NPI-efficacy approach)
# =============================================================================
...
#                  [min, max] intervals (Approach B; see NPI_SPEC below and
#                  npi_efficacy_from_scaler() in
#                  functions/abc_calibration_functions_new_approach.R).
#                    s = -1 -> min,  s = 0 -> central,  s = +1 -> max.
#
# FIXED (not fitted): general_hospital_quarantine_efficacy, safe_funeral_efficacy
#   (see FIXED_EFFICACIES). prob_hcw_cond_*_hospital are fixed at the honest 0.25
#   base — there is NO HCW-risk scalar in this approach.
#
# Observed summaries (3 + a "took off" indicator):
...
# New-approach helper functions live in the repo-level functions/ folder.
...
# Scalar overrides layered on DEFAULT_SCALAR_INPUTS. Pin the fixed efficacies
# here too so any direct/main-process model call is consistent with the fit.
MODEL_OVERRIDES <- c(
  list(check_final_size = 30000),
  FIXED_EFFICACIES
)
...
# ABC tuning. These travel into each worker via bootstrap_abc_worker(), which is
# why the NPI scaler spec and fixed efficacies are carried here.
ABC_CONFIG <- list(
...
# Invariants (Q_g, death-location probs, resolved t=0 prop_etu / unsafe-funeral)
# are efficacy-independent and computed ONCE. D and F are recomputed per particle
# inside build_abc_model_args().
...
# Sanity glance: D and F at the CENTRAL efficacies (s = 0).
...
# Back-transform npi_scaler -> implied efficacies for interpretation.
