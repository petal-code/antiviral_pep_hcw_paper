# DRC_run_abc_calibration_decoupled.R  (DECOUPLED-efficacy approach)
# =============================================================================
# ABC-SMC calibration of the revamped fiber branching-process model against the
# Middle_DRC_ConflictSmoothed_PlusPlus Ebola scenario, using the DECOUPLED scheme
...
#     450-day "duration". d_p05_p95 is the day span between the 5th and 95th
...
#     section-8 pairs() plot shows the ridge.
#
...
# DRC outbreak is ~3.5k cases; 10000 is comfortably above it.
MODEL_OVERRIDES <- list(check_final_size = 10000)

ABC_OUTPUT_BASE   <- ANALYSIS_DIR
ABC_OUTPUT_LABEL  <- "Decoupled"
FINAL_OUTPUTS_DIR <- here::here("outputs", "02_model_calibration_ABC")
...
ABC_SETTINGS <- list(
  ...
  alpha            = 0.5,
  tolerance_target = .prof$tolerance_target,  # don't chase below the noise floor; stop on plateau
  M                = 1,
  use_seed         = TRUE,
  verbose          = TRUE
)
