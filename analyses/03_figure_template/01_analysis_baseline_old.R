...
# Each worker runs: (N_PARTICLES / N_WORKERS) x 2 scenarios x N_REPS reps
# =============================================================================
...
source(here("functions", "abc_calibration_functions_npi.R"))
source(here("functions", "abc_calibration_functions_combined.R"))
source(here("functions", "abc_posterior.R"))

env_combined <- new.env(parent = globalenv())
sys.source(here("functions", "abc_calibration_functions_combined.R"), envir = env_combined)
...
SCENARIOS <- list(
  WestAfrica = list(
    ...
    rds          = here("outputs", "02_ABC_model_fits_NPI_Eff",
                        "Worst_WestAfrica_20260604_174540_Combined_NP4_NS5_NBREPS_70_NBSIMUL_460.rds"),
    scenario_csv = here("data-processed", "final_six_scenario_values_original_approach.csv"),
    param_names  = c("R0", "prop_funeral", "npi_scaler", "hcw_risk_scalar")
  ),
  DRC = list(
    id           = "Middle_DRC_ConflictSmoothed_PlusPlus",
    rds          = here("outputs", "02_ABC_model_fits_NPI_Eff",
                        "Middle_DRC_ConflictSmoothed_PlusPlus_20260603_222544_Combined_NP4_NS5_NBREPS_100_NBSIMUL_590.rds"),
    scenario_csv = here("data-processed", "final_six_scenario_values_original_approach.csv"),
    param_names  = c("R0", "prop_funeral", "npi_scaler", "hcw_risk_scalar")
  )
)
...
scenario_setups <- lapply(names(SCENARIOS), function(sc_name) {
  ...
  message(sprintf("  %s (combined fit)...", sc_name))
  ...
  build_combined <- env_combined$build_abc_model_args_combined
    ...
    for (sc_name in names(scenario_setups)) {
      ...
      for (r in seq_len(N_REPS)) {
        ...
        result <- list(
          ...
          npi_scaler       = setup$theta$npi_scaler[p],
