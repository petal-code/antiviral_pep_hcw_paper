# =============================================================================
# 01_analysis_figure4_new.R
#
# Runs antiviral simulations for Figure 4 (new): West Africa archetype only.
# Coverage swept from 5% to 100% (5% steps, 20 levels), combined with two
# fixed DPC values (0 and 5 days) and three efficacy arms (lo/mid/hi) derived
# from the DPC-efficacy lookup table at the corresponding fixed DPC.
#
# Arms (120 total):
#   cov{05-100}_dpc{0,5}_{mid,lo,hi}
#
# Coverage is constant over time (no ramp); DPC is constant over time (no
# conflict-driven trajectory) since this analysis is a static post-hoc
# sensitivity grid, not a time-varying scenario.
#
...
# =============================================================================
# 1. Coverage grid and fixed DPC levels
# =============================================================================
COVERAGE_LEVELS <- seq(0.05, 1.00, by = 0.05)   # 5% to 100%, 20 levels
DPC_LEVELS      <- c(0, 5)                       # fixed DPC values

# =============================================================================
# 2. Efficacy lookup: at each fixed DPC, read off lo/mid/hi efficacy
# =============================================================================
curve_d50_dat <- readRDS(here("data-processed", "DPC_fixed_efficacy_varied_d50.rds"))

get_efficacy_at_dpc <- function(dpc_val, col) {
  approx(x = curve_d50_dat$dpc, y = curve_d50_dat[[col]],
         xout = dpc_val, rule = 2)$y
}

EFFICACY_COLS <- c(mid = "efficacy", lo = "eighty_efficacy_lo", hi = "eighty_efficacy_hi")

# =============================================================================
# 3. Arm definitions: coverage x dpc x efficacy_arm
# =============================================================================
ARMS <- do.call(rbind, lapply(COVERAGE_LEVELS, function(cov) {
  do.call(rbind, lapply(DPC_LEVELS, function(dpc) {
    do.call(rbind, lapply(names(EFFICACY_COLS), function(eff_arm) {
      eff_val <- get_efficacy_at_dpc(dpc, EFFICACY_COLS[[eff_arm]])
      data.frame(
        arm_name = sprintf("cov%03d_dpc%d_%s", round(cov * 100), dpc, eff_arm),
        coverage = cov,
        dpc      = dpc,
        eff_arm  = eff_arm,
        efficacy = eff_val,
...
message(sprintf("Total arms: %d (%d coverage x %d dpc x %d efficacy)",
                nrow(ARMS), length(COVERAGE_LEVELS), length(DPC_LEVELS), length(EFFICACY_COLS)))
print(head(ARMS, 12))

# =============================================================================
# 4. Scenario: West Africa only
# =============================================================================
SCENARIOS <- list(
  WestAfrica = list(
    id           = "Worst_WestAfrica",
...
OUT_BASE <- here("outputs", "simulation", "figure4_new")
    ...
    for (sc_name in names(scenario_setups)) {
      ...
      for (arm_idx in seq_len(nrow(ARMS))) {
        ...
        for (r in seq_len(N_REPS)) {
          ...
          args$obv_pep_coverage         <- arm$coverage
          args$obv_pep_adherence        <- 1.0
          args$obv_pep_dpc              <- arm$dpc
