...
#
# Outputs:
#   figure_3new_weekly_ts.csv             : weekly HCW deaths, q025/q25/q50/q75/q975
#                                            pooled across (particle_id, rep)
#   figure_3new_particle_summary.csv      : deaths averted % per arm (pooled
#                                            across particle_id x rep; see
#                                            make_particle_df)
...
#   quantiles are taken directly across all (particle_id, rep) pairs, not
#   averaged within each particle first. This preserves replicate-level
#   (stochastic) uncertainty alongside particle-level (posterior-draw /
#   parameter) uncertainty in the reported interval.
ts_quantiles_3new <- raw_ts_3new %>%
  # Pad each (particle_id, rep) series to global_max_week with 0 incidence,
  # so the number of observations feeding each week's quantile stays
  # constant across the full time range (matters most at the tail, where
  # some arms' simulations end earlier than others).
  group_by(scenario, arm, metric) %>%
  group_modify(~ {
    all_weeks <- seq(min(.x$week), global_max_week, by = 7)
    tidyr::complete(.x,
                    tidyr::nesting(particle_id, rep),
                    week = all_weeks,
                    fill = list(value = 0))
  }) %>%
  ungroup() %>%
  arrange(scenario, arm, particle_id, rep, metric, week) %>%
  # cumsum for cumulative metric only, computed per (particle_id, rep) so
  # each replicate keeps its own trajectory; incidence stays as-is.
  group_by(scenario, arm, particle_id, rep, metric) %>%
  mutate(value = if (unique(metric) == "hcw_deaths") cumsum(value) else value) %>%
  ungroup() %>%
  # Pooled quantiles across ALL (particle_id, rep) pairs.
