# =============================================================================
# 02_plot_figure3_variants.R  -- three layout variants of the Figure-3 redesign
#   V1: weekly incidence (D,E) = OVERLAPPING bars (grey/No-antiviral at back,
#       shortest in front so every top shows); averted (F,G) = lines.
#   V2: incidence = lines; averted = STACKED bars + error bars.
#   V3: incidence = lines; averted = DODGED bars + error bars.
# Layout:  AAABBBCCC / DDDDDDFFF / EEEEEEGGG
# Prereqs: 02_extract_figure3.R has produced the two CSVs.
# =============================================================================
...
# ---- D,E as OVERLAPPING bars (back -> front by descending height) -----------
make_weekly_bars <- function(sc, panel_title) {
  xm <- x_max_weeks(sc)
  df <- weekly_80 %>% filter(scenario==sc, week<=xm) %>%
    mutate(line_group = factor(line_group, levels = c("baseline","ramp_low","ramp_high","full")))  # draw order
  ggplot(df, aes(week, q50, fill=line_group)) +
...
# ---- D,E as OVERLAPPING bars (one layer per series, back -> front) ----------
make_weekly_bars <- function(sc, panel_title) {
  xm <- x_max_weeks(sc)
  df <- weekly_80 %>% filter(scenario == sc, week <= xm) %>%
    mutate(line_group = factor(line_group, levels = LINE_LEVELS))   # for fill + legend
  back_to_front <- c("baseline", "ramp_low", "ramp_high", "full")   # tallest -> shortest (front)
  layers <- lapply(back_to_front, function(g)
    geom_col(data = df[df$line_group == g, ], aes(week, q50, fill = line_group),
...
averted_summary <- function(sc, metric="pct_hcw_deaths_averted") {
  pdf %>% filter(scenario==sc, eff_name %in% FIG3_EFFICACY_LEVELS, !is.na(.data[[metric]])) %>%
...
# ---- F,G as LINES -----------------------------------------------------------
make_averted_lines <- function(sc, y_label="HCW deaths averted") {
  ggplot(averted_summary(sc), aes(efficacy_pct, med, color=coverage_label, fill=coverage_label)) +
    geom_ribbon(aes(ymin=lo, ymax=hi), alpha=0.15, color=NA) + geom_line(linewidth=1) + geom_point(size=1.6) +
    scale_color_manual(values=.cov_fill, name=NULL) + scale_fill_manual(values=.cov_fill, name=NULL) +
    scale_x_continuous(breaks=EFF_NUM, labels=function(x) paste0(x,"%")) +
    scale_y_continuous(labels=function(x) paste0(x,"%")) +
    guides(color="none", fill="none") + lims(y=c(0,NA)) +
    labs(x="Antiviral efficacy", y=y_label) + theme_fig()
}

# ---- F,G as STACKED bars + error bars ---------------------------------------
# NB: stacking averted-% across scenarios isn't additive in meaning -- here only
# to compare layouts. Error bars = each scenario's 95% CrI in stacked coords.
make_averted_stacked <- function(sc, y_label="HCW deaths averted") {
  d <- averted_summary(sc) %>% arrange(efficacy_pct, coverage_label) %>%
    group_by(efficacy_pct) %>%
    mutate(cum_top=cumsum(med), cum_bot=cum_top-med, err_lo=cum_bot+lo, err_hi=cum_bot+hi) %>% ungroup()
  ggplot(d) +
    geom_rect(aes(xmin=efficacy_pct-4, xmax=efficacy_pct+4, ymin=cum_bot, ymax=cum_top, fill=coverage_label),
              color="white", linewidth=0.2) +
    geom_errorbar(aes(x=efficacy_pct, ymin=err_lo, ymax=err_hi), width=2.5, color="grey30", linewidth=0.4) +
...
assemble <- function(de_fun, fg_fun, file) {
...
fig_v1 <- assemble(make_weekly_bars,  make_averted_lines,   "figure_3_v1_bar-incidence.pdf")
fig_v2 <- assemble(make_weekly_bars,  make_averted_dodged,   "figure_3_v1_bar-incidence.pdf")
fig_v3 <- assemble(make_weekly_lines, make_averted_dodged,  "figure_3_v3_dodged-averted.pdf")
message("Saved figure_3 variants: v1 (bar incidence), v2 (stacked averted), v3 (dodged averted)")
