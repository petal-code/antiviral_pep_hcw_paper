...
#
# Pooled aggregation (variance-compression fix): previously this grouped by
# (scenario, particle_id, arm, onset_day, max_cov, efficacy) -- omitting
# `rep` -- and summed each replicate's counts within a particle BEFORE
# computing percentages, then took quantiles across only the resulting
# particle-level values downstream. That collapsed replicate-level
# (stochastic) uncertainty before the quantile step. make_particle_df() now
# keeps each (particle_id, rep) as its own row, so downstream quantiles
# (heatmap medians, sens_text CIs) are pooled across all replicates.
# =============================================================================
particle_df <- make_particle_df(sens_raw) %>%
