...
decomp_particle <- particle_3new %>%
  ...
  # NOTE: particle_3new is now pooled at (particle_id, rep) granularity
  # (see make_particle_df variance-compression fix). `rep` must stay in the
  # pivot key, otherwise the N_REPS replicates sharing the same particle_id
  # collide on the same (particle_id, eff_arm, cov_scen) key and
  # pivot_wider silently mismatches them. Keeping rep also correctly
  # preserves the matched-seed pairing between optimistic/dpc_conflict/
  # with_conflict for the same stochastic realisation.
  select(particle_id, rep, eff_arm, cov_scen, pct_hcw_deaths_averted) %>%
