...
# (scenario, particle_id, arm, obv_efficacy, obv_coverage) -- omitting `rep`
# -- and averaged/summed each replicate's counts within a particle BEFORE
# computing percentages, then took the median across only the resulting
# particle-level values. make_particle_df() now keeps each (particle_id,
# rep) as its own row, so the median below is pooled across all replicates.
heatmap_df <- make_particle_df(run_df) %>%
...
# Pooled aggregation (variance-compression fix): see note above heatmap_df.
make_particle_df(run_df4) %>%
  filter(
