...
compute_doses_per_death <- function(run_df,
  ...
  # Pooled aggregation (variance-compression fix): doses_per_death is computed
  # per (particle_id, rep) -- i.e. per stochastic replicate -- and quantiles
  # are taken directly across all of those replicates. Previously this
  # averaged doses/deaths_averted over reps within each particle_id BEFORE
  # computing doses_per_death and its quantiles, which compressed
  # replicate-level stochastic variance out of the reported interval.
  results %>%
