...
# make_particle_df
#
# IMPORTANT (variance compression fix):
#   `run_df` (from extract_run_summary) has one row per stochastic replicate
#   (i.e. one row per particle_id x rep). Previously this function collapsed
#   the N_REPS replicates within each particle via mean()/sum() BEFORE
#   percentages were computed, and downstream code then took quantiles across
#   only the resulting particle-level values. That two-step "average reps,
#   then quantile across particles" procedure silently drops replicate-level
#   (stochastic) uncertainty and only carries forward parameter (posterior
#   draw) uncertainty -- this compresses the width of any downstream
#   quantile/credible-interval summary.
#
#   By default (pool_reps = TRUE) this function no longer collapses reps: it
#   returns one row per (scenario, particle_id, rep, arm), with percentages
#   computed directly at the replicate level. Downstream quantile summaries
#   computed on this pooled data reflect BOTH parameter uncertainty (variation
#   across particle_id / posterior draws) and stochastic uncertainty
#   (variation across rep for a fixed particle_id).
#
#   pool_reps = FALSE reproduces the old (pre-fix) behaviour -- replicates are
#   averaged/summed within each particle first. This is kept only for
#   diagnostic comparison and should not be used for reported results.
# =============================================================================
make_particle_df <- function(run_df, pool_reps = TRUE) {
  df <- run_df %>%
    mutate(
  ...
  if (pool_reps) {
    # Pooled (recommended): keep replicate-level granularity. Every row is
    # a single stochastic replicate; quantiles computed on this downstream
    # are pooled over (particle_id, rep).
    return(df)
  }

  # ---- Legacy / diagnostic only: collapse reps within each particle first ----
  run_df %>%
