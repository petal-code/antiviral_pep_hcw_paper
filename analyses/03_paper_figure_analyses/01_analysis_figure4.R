# =============================================================================
# 01_analysis_figure4.R
#
# Runs OBV simulations exclusively for constant coverage grid combinations.
...
ARMS <- do.call(rbind, lapply(CONST_COVERAGES, function(cov) {
...
message(sprintf("Total arms: %d (const grid: %d)", nrow(ARMS), nrow(ARMS)))
print(ARMS$arm_name)
  ...
  for (p in seq(job$p_from, job$p_to)) {
    for (sc_name in names(scenario_setups)) {
      ...
      for (arm_idx in seq_len(nrow(ARMS))) {
        ...
        # Resolve coverage function (Only Constant Grid in this script)
