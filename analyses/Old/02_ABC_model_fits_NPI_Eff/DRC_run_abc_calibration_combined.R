# DRC_run_abc_calibration_combined.R  (COMBINED approach: NPI-efficacy + HCW-risk)
# =============================================================================
# ABC-SMC calibration of the revamped fiber branching-process model against the
# DRC Ebola outbreak (Middle_DRC_ConflictSmoothed_PlusPlus), using the COMBINED
# parameterisation (functions/abc_calibration_functions_combined.R) that unions
# the NPI-efficacy and HCW-risk schemes.
#
# Two things differ from DRC_run_abc_calibration_peak.R:
#   (1) The fitted SUMMARY set DROPS the near-saturated `takeoff` and keeps the
#       curve-shape feature `peak_height`, giving FOUR informative summaries:
#         n_deaths, n_hcw_deaths, duration, peak_height
#       (peak_height breaks the R0 <-> etu size-confound that pure n_deaths leaves).
#   (2) `hcw_risk_scalar` is available as a FOURTH fittable parameter (it scales
#       the symmetric prob_hcw_cond_*_hospital base), so HCW deaths get a lever
#       that is near-orthogonal to overall size.
#
...
#         -> the pure NPI-efficacy fit (== DRC_run_abc_calibration[_peak].R).
...
#     observed peak_height target are PLACEHOLDER values. UPDATE WITH REAL NUMBERS
#     (peak weekly deaths from the DRC weekly surveillance series; literature
#     bounds for the efficacies/HCW exposure) before any production run.
# =============================================================================
...
# a tight diagonal band = a confounded pair. Weighted by posterior weights.
