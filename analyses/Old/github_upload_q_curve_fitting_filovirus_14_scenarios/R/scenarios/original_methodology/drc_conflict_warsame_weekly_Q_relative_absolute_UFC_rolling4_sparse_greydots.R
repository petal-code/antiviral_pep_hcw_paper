...
out_prefix <- "drc_conflict_warsame_weekly_Q_relative_absolute_UFC_rolling4_sparse_greydots"
...
p_main <- ggplot(curve_summ, aes(x = relative_day, y = mean)) +
  ...
  theme_bw(base_size = 11) +
  labs(
    title = "DRC conflict scenario: empirical weekly SDB curve, Q start at 0, max-scaled",
    subtitle = paste0("Q is relative for response parameters; p_unsafe_funeral_comm is absolute 1 - successful_response; grey circles show sparse 4-week rolling-average absolute UFC proxy; aggregation=", aggregation_unit, "; smoothing=", q_smoothing_method, "; DRC_UFC_00 and DRC_UFC_01 reference-only"),
...
p_q <- ggplot(q_shape_summ, aes(x = relative_day, y = mean)) +
  ...
  theme_bw(base_size = 11) +
  labs(
    title = "DRC conflict scenario: shared empirical Q shape applied to all response parameters",
    subtitle = paste0("Q is the empirical averaged successful-response trajectory, where success includes SDB not needed, empirically max-scaled after adding Q(0)=0; aggregation=", aggregation_unit, "; smoothing=", q_smoothing_method),
    x = "Relative outbreak day",
