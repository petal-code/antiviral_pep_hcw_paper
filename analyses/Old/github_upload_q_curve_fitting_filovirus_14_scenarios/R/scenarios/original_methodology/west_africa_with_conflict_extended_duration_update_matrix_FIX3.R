...
new_sheet_name    <- "Worst_WestAfrica_Conflict"
new_scenario_name <- "worst_west_africa_conflict"
...
out_prefix <- "west_africa_with_conflict_extended_DRC_Q_modulated"
...
p_theta <- ggplot(hybrid_curve, aes(x = relative_day, y = mean)) +
  ...
  theme_bw(base_size = 11) +
  labs(
    title = "Extended hypothetical West Africa-with-conflict parameter trajectories",
    subtitle = "Original West Africa start/end magnitudes retained; DRC conflict-modulated Q shape imposed over extended duration",
    x = "Extended West Africa relative outbreak day",
    y = NULL
  ) +
  theme(
