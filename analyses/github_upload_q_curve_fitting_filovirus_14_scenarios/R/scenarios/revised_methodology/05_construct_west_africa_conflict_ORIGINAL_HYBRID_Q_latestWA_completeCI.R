...
out_prefix <- "west_africa_with_conflict_originalHybridQ_latestWA_completeCI"
scenario_name <- "Worst_WestAfrica_Conflict"
...
p_theta <- ggplot(hybrid_curve, aes(x = relative_day, y = mean)) +
  ...
  theme_bw(base_size = 11) +
  labs(
    title = "Extended hypothetical West Africa-with-conflict parameter trajectories, complete CI",
