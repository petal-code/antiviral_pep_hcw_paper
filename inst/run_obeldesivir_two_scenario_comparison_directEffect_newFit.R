# =============================================================================
# run_obeldesivir_two_scenario_comparison_directEffect_newFit.R
#
# NPI-efficacy-fit variant of run_obeldesivir_two_scenario_comparison_directEffect.R.
# Identical to that script except it loads the NEW (R0, prop_funeral, npi_scaler)
# fit and converts npi_scaler -> efficacies (everything downstream is unchanged).
#
# Estimates the % of healthcare-worker (HCW) deaths that obeldesivir (OBV)
# post-exposure prophylaxis averts, for the fitted scenario(s) and at TWO OBV
...
#   Rscript inst/run_obeldesivir_two_scenario_comparison_directEffect_newFit.R
# =============================================================================
...
# The scenarios: a friendly label -> (NPI-efficacy posterior RDS in
# outputs/02_ABC_model_fits_NPI_Eff/, scenario id). Only the DRC PlusPlus NPI fit
# exists so far.
SCENARIOS <- list(
  DRC        = list(rds = "fiber_ABC_SMC_Middle_DRC_ConflictSmoothed_PlusPlus_NPIeff_20260601_154658.rds",
                    id  = "Middle_DRC_ConflictSmoothed_PlusPlus")
)

# Calibration settings reproduced so the posterior -> model-args conversion is
# identical to how the NPI ABC was run (see analyses/02_ABC_model_fits_NPI_Eff/).
# npi_scaler in [-1, 1] indexes these [min, max] efficacy intervals (the npi_spec
# the fit used; ppe 0.30-0.90, etu 0.60-0.95); see npi_efficacy_from_scaler().
NPI_SPEC <- list(ppe_efficacy = list(min = 0.30, max = 0.90),
...
source(file.path(FN, "abc_calibration_functions_npi.R"))     # build_abc_model_args()
...
# ---------------------------------------------------------------------------
# The fit files are EasyABC result objects: $param (3 columns: R0,
# prop_funeral, npi_scaler) and $weights. Rebuild the weighted-posterior
# data frame downsample_posterior() expects.
posteriors <- lapply(SCENARIOS, function(sc) {
  res <- readRDS(here::here("outputs", "02_ABC_model_fits_NPI_Eff", sc$rds))
...
scenario_matrix  <- read_scenario_matrix(here::here("data-processed", "final_six_scenario_values_original_approach.csv"))
...
for (sc in names(SCENARIOS)) {
  ...
  # Efficacy-independent R0 invariants for THIS scenario (D/F are recomputed per
  # particle inside build_abc_model_args, since etu_efficacy is now fitted).
